...
function UploadJobRow({ job }: { job: UploadJob }) {
  ...
  return (
    <div className="bg-ey-surface border border-ey-border rounded p-4">
      ...
      {isError && (
        <p className="text-xs text-red-400 line-clamp-2 break-words">
          {job.error ?? 'Upload failed. Please try again.'}
        </p>
      )}
    </div>
  )
...
export default function UploadPanel({ onClose, onContractAdded }: Props) {
    ...
    const handle = setInterval(async () => {
      try {
        ...
        if (status.status === 'done') {
          ...
          clearInterval(handle)
          pollers.current.delete(jobId)
          updateJob(jobId, { stage: 'error', error: (status.error ?? 'Ingestion failed.').slice(0, 160) })
        }
      } catch (err) {
        clearInterval(handle)
        pollers.current.delete(jobId)
  ...
  useEffect(() => {
    ...
    api.listIngestJobs()
      .then(serverJobs => {
        if (cancelled) return
        // Show only the 8 most recent jobs so old history doesn't flood the pane.
        const recent = serverJobs.slice(0, 8)
        const mapped: UploadJob[] = recent.map(j => ({
          id: j.jobId,
          fileName: j.fileName,
          fileSize: '',
          stage: toUploadStage(j.stage),
          progress: j.progress,
          error: j.error ? j.error.slice(0, 160) : undefined,
        }))
        setJobs(mapped)
