...
def _run_kg_pipeline(
    ...
    # Demo / low-quota switch: skip all KG extraction so no background LLM
    # calls compete with live chat for Azure OpenAI quota. Retrieval falls
    # back to tree-only search automatically.
    if getattr(config, "DISABLE_KG_EXTRACTION", False):
        logger.info("Job %s: KG extraction disabled by config, skipping.", job_id)
        return False
    ...
    try:
        ...
        try:
            # Cap clauses to KG_EXTRACTION_LIMIT so a large contract can't fire
            # hundreds of background LLM calls and exhaust the chat quota.
            job_store.update_stage(job_id, user_id, stage="extracting", progress=82)
            _limit = getattr(config, "KG_EXTRACTION_LIMIT", 20) or None
            clauses = select_representative_clauses(normalized.nodes, limit=_limit)
...
def _run_job(
    ...
    try:
        ...
        # Full detail (with traceback) goes to logs; the user-facing job record
        # gets a short, single-line message so the upload panel stays readable.
        error_msg = f"{type(exc).__name__}: {exc}\n{traceback.format_exc()}"
        logger.error("Job %s failed: %s", job_id, error_msg)
        short_error = f"{type(exc).__name__}: {exc}".splitlines()[0][:200]
        job_store.mark_failed(job_id=job_id, user_id=user_id, error=short_error)
