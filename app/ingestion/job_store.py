...
Container: ingest_jobs  (partition key = /jobId)
...
class JobStore:
    ...
    def ensure_container(self) -> None:
        ...
        db.create_container_if_not_exists(
            id=_CONTAINER_NAME,
            partition_key=PartitionKey(path="/jobId"),
    ...
    def get_job(self, job_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        try:
            return self._container.read_item(item=job_id, partition_key=job_id)
    ...
    def list_jobs(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        return self._reap_stale(jobs)

    def _reap_stale(self, jobs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Relabel jobs stuck in 'queued'/'processing' past the staleness window as
        failed. These are orphaned runs (e.g. the worker container crashed
        mid-job), which would otherwise spin forever in the upload panel.
        The change is persisted so it self-heals across reloads.
        """
        threshold_min = getattr(config, "JOB_STALE_AFTER_MINUTES", 15)
        cutoff = datetime.now(timezone.utc) - timedelta(minutes=threshold_min)
        for job in jobs:
            if job.get("status") not in ("queued", "processing"):
                continue
            try:
                updated = datetime.fromisoformat(job.get("updatedAt", ""))
            except ValueError:
                continue
            if updated < cutoff:
                job["status"] = "failed"
                job["stage"] = "error"
                job["progress"] = 0
                job["error"] = "Ingestion timed out or was interrupted."
                job["updatedAt"] = _now()
                try:
                    self._container.replace_item(item=job["id"], body=job)
                except Exception:
                    pass
        return jobs
