...
def answer_question(
    ...
    # ── Two-stage contract scoping ────────────────────────────────────
    # Stage 1: the UI selection (contract_ids multi-select, or contract_id
    # session filter, or the whole portfolio) forms the candidate set.
    # Stage 2: narrow that set to the contract(s) the question names.
    ...
    if contract_ids:
        ...
        # Portfolio-wide: pull the full indexed list so a question that names a
        # contract ("...in the Edison contract") can still self-scope.
        try:
            candidate_pool = AzureSearchTester().list_contract_ids()
        except Exception as exc:  # pragma: no cover - best-effort narrowing
    ...
    if scope_reason:
        # Narrowing actually happened — apply it as the effective scope.
    ...
    # Surface question-based narrowing in the reason shown to the user.
    if scope_reason:
